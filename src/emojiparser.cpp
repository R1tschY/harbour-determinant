...
static Q_LOGGING_CATEGORY(logger, "determinant.emojiparser");
...
static QString toIconId(const QStringRef& s) {
    ...
    bool containsZwj = s.contains(QChar(ZERO_WIDTH_JOINER));

    const QChar* iter = s.cbegin();
    const QChar* last = s.cend();
    ...
    for (; iter < last; ++iter) {
        ...
        if (QChar::isHighSurrogate(c)) {
            ++iter;
            if (iter < last) {
                result.append(QString::number(QChar::surrogateToUcs4(c, iter->unicode()), 16));
            } else {
                // missing low surrogate
            }
        } else {
            if (containsZwj || c != VARIATION_SELECTOR_16) {
                result.append(QString::number(c, 16));
            }
        }
    }
...
QString EmojiParser::replace(const QStringRef &match, const QString& emojiSize)
{
    return QStringLiteral("<img src=\"")
            % m_twemojiPrefix
            % QStringLiteral("/svg/")
            % toIconId(match)
            % QStringLiteral(".svg\" align=\"middle\" width=\"")
            % emojiSize
